...
Internally this searches a file containing the reformatted
...
used instead by using the B<-F> (Fixed String) option.
...
Limit search to packages for one architecture
...
to enable/disable gzip compression of F</var/lib/dlocate/dlocatedb>. This
option is obsolete and unsupported unless someone wants to provide a patch (shouldn't be too hard with, say, B<IO::Compress::Gzip>)

=item F</var/lib/dlocate/dlocatedb>

Plain text listing of all files belonging to all packages (previously,
this was optionally compressed but B<dlocate> now supports
